...
def build_sample_db():
    ...
    for i in range(len(first_names)):
        ...
        user.password = generate_password_hash(user.first_name.lower())
        # user.password = generate_password_hash(''.join(random.choice(string.ascii_lowercase + string.digits) for i in range(10)))
        db.session.add(user)

    es = User(login = "vahag", password = generate_password_hash("vahag"))
    db.session.add(es)
    Avik = User(login = "avik", password = generate_password_hash("avik"))
    db.session.add(Avik)
